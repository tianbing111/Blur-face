...
# dimensions
for filename in os.listdir(r"examples"):
    print(filename)
    image = cv2.imread("examples/"+filename)
    orig = image.copy()
    (h, w) = image.shape[:2]
    ...
    cv2.imwrite("result/{}_result.png".format(filename),output,[int(cv2.IMWRITE_PNG_COMPRESSION),5])
